...
def tag_records(records, model="qwen-plus", show_progress=True):
    ...
    rows = []
    client = _make_client()
    for idx, r in enumerate(tqdm(records, desc="Processing")):
        ...
        tag, reason = _classify_one(client=client, model="qwen-plus", content=content, temperature=0.0, max_retries=4)
        rows.append({
            "id": r.get("id"),
            "instruction": instruction,
            "context": context,
